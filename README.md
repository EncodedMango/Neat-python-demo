...
There are 250 players, each of them starts at the center, there is a terminator (large magenta dot) that eliminates the players one by one. Each player is given its current direction, distance to the
world border (red circle), the direction the terminator is in, and the direction to the nearest goal (magenta dots).

Each player has a definite life-span of 400 loops, when it reaches a goal this is reset to 0.
